...
AP_OpticalFlow_ADNS3080::AP_OpticalFlow_ADNS3080(int cs_pin, int reset_pin) : _cs_pin(cs_pin), _reset_pin(reset_pin)
{
	...
	_motion = false;
	_overflow = false;
	pixels_x = ADNS3080_PIXELS_X;
	pixels_y = ADNS3080_PIXELS_Y;
}

// Public Methods //////////////////////////////////////////////////////////////

// init - initialise sensor
// initCommAPI parameter controls whether SPI interface is initialised
// (set to false if other devices are on the SPI bus and have already initialised the interface)

bool AP_OpticalFlow_ADNS3080::init(bool initCommAPI)
{
    int retry = 0;
	...
	if( initCommAPI ) {
    	...
    	SPI.setDataMode(SPI_MODE3);
    	 // sensor running at 2Mhz.  this is it's maximum speed
    	SPI.setClockDivider(SPI_CLOCK_DIV32);
	}

	// check the sensor is functioning
	while( retry < 3 ) {
	    if( read_register(ADNS3080_PRODUCT_ID) == ADNS3080_PRODUCT_ID_VAL )
	        return true;
	    retry++;
	}
...
bool AP_OpticalFlow_ADNS3080::update()
{
	...
	if( (motion_reg & 0x80) != 0 ) {
		motion_reg = read_register(ADNS3080_DELTA_X);
		sign_bit = ((motion_reg & 0x80) != 0);
		if(sign_bit){
			motion_reg = ~motion_reg;
			raw_dx = (-1)*(int)motion_reg;
		} else {
			raw_dx = (int)motion_reg;
		}
		// small delay
		delayMicroseconds(100);
		motion_reg = read_register(ADNS3080_DELTA_Y);
		sign_bit = ((motion_reg & 0x80) != 0);
		if(sign_bit){
			motion_reg = ~motion_reg;
			raw_dy = (-1)*(int)motion_reg;
		} else {
			raw_dy = (int)motion_reg;
		}
...
void AP_OpticalFlow_ADNS3080::print_pixel_data(Stream *serPort)
{
    int i,j;
